...
def get_shortest_paths_teams(player_name):
    ...
    # json.dump(
    #     player_node.to_dict(),
    #     open(f'paths_teams_{player_name.lower().replace(" ", "_")}.json', "w"),
    # )
    return player_node
    ...
    if len(paths) == 1:
        ...
        if "full_name" in player_graph.nodes[paths[0]]:
            ...
            data.append(
                {
                    "id": paths[0],
                    "children": create_nested_structure([], paths[1:], player_graph),
                    "name": player_name,
                }
            )
        return data
...
def create_nested_structure_teams(
    player_node: PlayerNode, paths, player_graph
) -> PlayerNode:
    if len(paths) == 1:
        return player_node
    if len(paths) == 2:
        ...
        if "team_name" in team_edge and "full_name" in player:
            ...
            for team in player_node.teams:
                if team.id == team_id:
                    team.players.append(
                        PlayerNode(
                            paths[1], player_graph.nodes[paths[1]]["full_name"], []
                        )
                    )
                    return player_node
            ...
            node = TeamNode(
                team_edge["team_name"], team_edge["season"], [new_player_node]
            )
                ...
                if team.id == team_id:
                    for team_player in team.players:
                        if team_player.id == paths[1]:
                            team_player = create_nested_structure_teams(
                                team_player, paths[1:], player_graph
                            )
                            return player_node

                    new_player_node = PlayerNode(paths[1], player["full_name"], [])
                    new_player_node = create_nested_structure_teams(
                        new_player_node, paths[1:], player_graph
                    )
